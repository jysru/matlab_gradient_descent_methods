function [x_opt, fval, history] = NesterovAcceleratedGradient(func, x0, alpha, opts)
    ...
    for iter = 1:opts.maxIter
        % Lookahead step
        lookahead_x = x - opts.beta * v;

        % Compute the numerical gradient
        [grad, fevals] = opts.grad_func(func, lookahead_x, opts.epsilon);

        % Update the variables using the gradient descent step
        [x_new, v] = StreamOptim.Optims.Steps.NesterovAcceleratedGradientStep(x, grad, v, alpha=alpha, beta=opts.beta);
